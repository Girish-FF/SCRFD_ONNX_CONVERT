...
def pytorch2onnx(config_path,
    ...
    # Recursive fix for any nested numpy arrays
    def convert_to_tensor(obj):
        """Recursively convert numpy arrays to torch tensors inside any nested structure."""
        if isinstance(obj, np.ndarray):
            return torch.from_numpy(obj).float()
        elif isinstance(obj, (list, tuple)):
            return type(obj)(convert_to_tensor(x) for x in obj)
        else:
            return obj

    tensor_data = convert_to_tensor(tensor_data)
